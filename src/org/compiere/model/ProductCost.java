...
public class ProductCost
{
	...
	public MAccount getAccount(int AcctType, MAcctSchema as)
	{
		...
		try
		{
			...
			// Xpande. Gabriel Vila. 08/12/2020.
			// Si no encuentra cuenta en producto, tiene que buscarla en la caterogría de producto (subir un nivel)
			// Comento codigo original y sustiyuyo
			...
			*/
			...
			if (validCombination_ID <= 0){
				int mProductCategoryID = this.getProduct().getM_Product_Category_ID();
				if (mProductCategoryID > 0){
					return this.getAccountCategory(AcctType, mProductCategoryID, as);
				}
			}
	...
	/***
	 * Retorna cuenta contable según ID de categoría de producto.
	 * Xpande. Created by Gabriel Vila on 12/8/20.
	 * @param AcctType
	 * @param mProductID
	 * @param as
	 * @return
	 */
	public MAccount getAccountCategory (int AcctType, int mProductCategoryID, MAcctSchema as)
	{
		if (AcctType < 1 || AcctType > 23)
			return null;
		...
		String sql = "SELECT P_Revenue_Acct, P_Expense_Acct, P_Asset_Acct, P_Cogs_Acct, "
				...
				+ "WHERE M_Product_Category_ID =" + mProductCategoryID
				+ " AND C_AcctSchema_ID=? ";
		//
		int validCombination_ID = 0;
		PreparedStatement pstmt = null;
