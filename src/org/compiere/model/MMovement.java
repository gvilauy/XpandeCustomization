...
public class MMovement extends X_M_Movement implements DocAction, DocOptions
{
	 ...
	 * Xpande. Created by Gabriel Vila on 3/18/21.
	 * @param docStatus
	 * @param processing
	 * @param orderType
	 * @param isSOTrx
	...
	public boolean reActivateIt()
	{
		...
		// Xpande. Gabriel Vila. Agrego funcionalidad para reactivar.
		...
		for (MMovementLine movementLine : movementLines)
		{
			...
			String action = " delete from " + X_M_MovementLineMA.Table_Name + " where m_movementline_id =" + movementLine.get_ID();
			DB.executeUpdateEx(action, get_TrxName());

			// Eliminar info en m_transaction
			action = " delete from m_transaction where m_movementline_id =" + movementLine.get_ID();
			DB.executeUpdateEx(action, get_TrxName());
		...
		// Xpande. Gabriel Vila. 09/10/2018..
