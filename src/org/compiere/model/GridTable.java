	...
	implements Serializable
{
	...
	public char dataSave (boolean manualCmd)
	{
		...
		// Xpande. Gabriel Vila.
		// Cuando se carga info desde un smartbrowser pasa que se pierde el valor de AutoCommit (grabar sin consultar)
		// Decido comentar este código para que no pida nunca confirmación para guardar cambios.
		/*
	...
	private char dataSavePO (int Record_ID) throws Exception
	{
		...
		for (int col = 0; col < size; col++)
		{
			...
			else if (m_inserting || isValueChanged(oldValue, value) )
			{
				...
				// XPande. Gabriel Vila. 17/06/2017.
				// Cuando tengo una columna definida en la tabla como Entero de 10 y es un ID (ejemplo C_Currency_ID_SO),
				// y en el campo de la ventana le pongo referencia Tabla, sucede lo siguiente:
				// cuando saco el valor, y quiero guardar el registro me sale el siguiente error:
				// Z_PautaComercialSet_ID_1= 1000000(java.lang.String) != DB: 1000000(java.lang.Integer) -> New: null
				// Agrego este código para evitar el bug
				if (dbValue != null){
					if (dbValue.getClass().equals(Integer.class)){
						if(oldValue != null) {
