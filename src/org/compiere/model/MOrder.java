...
public class MOrder extends X_C_Order implements DocAction
{
	...
	public boolean voidIt()
	{
		...
		// No manejo documentos reversados.
		// Comento código y sustituyo.
	...
	/***
	 * Anula documentos InOut e Invoices que estén asociadas a esta orden de venta.
	 * Xpande. Created by Gabriel Vila on 1/2/19.
	 * @return
	 */
	private boolean voidLinkedDocuments()
	{
		...
		// No dejo anular esta orden si tiene al menos una factura de venta completa.
		MInvoice[] invoices = getInvoices();
		for (int i = 0; i < invoices.length; i++)
		{
			MInvoice invoice = invoices[i];
			...
			if (MInvoice.DOCSTATUS_Completed.equals(invoice.getDocStatus()))
			{
				m_processMsg = "No es posible anular esta Orden de Venta ya que tiene una Factura de Venta Completa : " + invoice.getDocumentNo();
				return false;
			}
			else{
		...
		for (int i = 0; i < shipments.length; i++)
		{
			...
			/*
			if (!ship.voidIt()){
				m_processMsg = ship.getProcessMsg();
				return false;
			}
			*/
