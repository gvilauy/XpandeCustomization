...
public class MOrderLine extends X_C_OrderLine implements IDocumentLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		else	//	Set/check Product Price
		{
			...
			{
				// Xpande. Gabriel Vila. 17/10/2019.
				// Antes de tirar la excepcion de que este producto no tiene precio en la lista de precios de la orden,
				// Pregunto si en la configuración comercial esta configurado para chequear o no esto.
				// Si no debo chequear entonces no tiro la excepcion y acepto precio CERO para el usuario lo digite ahora.
				// Comento la exception original y sustiyuyo.

				//throw new ProductNotOnPriceListException(m_productPrice, getLine());

				boolean chequearPrecio = false;

				try{
					String sql = " select coalesce(checkpriceinlist,'N') as chequeo from z_comercialconfig where value='General' ";
					String value = DB.getSQLValueStringEx(get_TrxName(), sql);
