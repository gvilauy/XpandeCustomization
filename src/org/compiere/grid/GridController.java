		...
		VetoableChangeListener,	PropertyChangeListener, MouseListener
{
	...
	private int setupVTable (APanel aPanel, GridTab mTab, VTable table)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			// Xpande. Gabriel Vila. 25/07/2018. Utiliza despliegue lógico de este GridField para determinar si muestro o no en grilla.
			else{
				if ((mField.getDisplayLogic() != null) && (!mField.getDisplayLogic().trim().equalsIgnoreCase(""))){
					boolean result = Evaluator.evaluateLogic(mField, mField.getDisplayLogic());
					if (!result){
						vTable.setColumnVisibility(tc, false);
					}
				}
	...
	public void activate ()
	{
		...
		// Xpande. Gabriel Vila. 25/07/2018. Lógica de despliegue para visibilidad de columnas de grilla.
		// Comento linea original y sustituyo.

		//if (!init) init();    // Linea original

		if (!init){
			init();
		}
		else{
			if (m_mTab.isDisplayGridRefresh()){
				this.setVisibilidadColumnasGrid();
			}
		}
		// Fin Xpande
	...
	/***
	 * Metodo para refrescar visibiliadd de columnas en grilla según logica de despliegue de cada field.
	 * Xpande. Created by Gabriel Vila on 7/25/18.
	 */
	private void setVisibilidadColumnasGrid()
	{
		//  Set up Multi Row Table
		vTable.setModel(m_mTab.getTableModel());
